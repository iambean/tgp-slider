...
!function (window, undefined) {
    ...
    //     //'3d', 'single-fade', 'single-switch'
    //     "CONTENT" : {
    //         "single"        : "single", // 单张图左右切换。默认效果。
    //         "fill"          : "fill",   // 铺满，拉伸时缩放图片以填满viewport
    //         "tiled"         : "tiled",  // 平铺，拉伸时不缩放图片，图片占位的高宽固定。
    //         "fade"          : "fade",    // 单张图淡入淡出效果
    //         "multi"         : "",               // slider的一帧包含多张图拼成。一般搭配fill使用，保证拉伸时不变形。
    //         "slider-3d"            : "slider-3d",      // 三张图模拟3d切换的效果
    //
    ...
    Slider.prototype = {
        ...
        setItemData: function setItemData(liEle, itemIndex) {
            ...
            if (Array.isArray(arguments[0])) {
                var _ret = function () {
                    var slider = _this;
                    [].forEach.call(_arguments, function (argItem) {
    ...
    function _make_skeleton(slider) {
        ...
        if (options.ownClass) {
            sliderClass.push(options.ownClass);
        }
    ...
    function bindEvent(ele, eName, agent, fn) {
        ...
        if (ele instanceof NodeList || ele instanceof HTMLCollection) {
            var _ret7 = function () {
                ...
                var args = [].slice.call(_arguments2, 0);
